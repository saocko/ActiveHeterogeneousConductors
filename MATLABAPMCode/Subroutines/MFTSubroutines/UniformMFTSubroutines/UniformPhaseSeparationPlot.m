...
if(sum(gradient(log(cond_evap_ratios))>0) == 0)
    ...
    for i = 1:length(ok_first_indices)
        ...
        if(length(pol_roots)>0)
            ...
            orig_free_energy = accum_array(dens_info.base_index) - padded_density_list(dens_info.base_index) * current_choice.lograt; %The free energy of the unseparated state
